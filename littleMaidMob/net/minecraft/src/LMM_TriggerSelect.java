 ...
 * マルチ対策用。
 * データの読み込みはIFFで行っている。
 * Container of mode switching trigger item.
 * Multi countermeasure.
 * I have done in the IFF is reading data.
 */
public class LMM_TriggerSelect {
	...
	public static Map<Integer, List<Integer>> getUserTrigger(String pUsername) {
		...
		if (MMM_Helper.isLocalPlay()) {
			// シングル実行時は名称ブランクに。, The name blank single runtime.
			pUsername = "";
		}
		// 存在チェック、無かったら追加, Add presence check, If it were not
		if (!usersTrigger.containsKey(pUsername)) {
			if (pUsername.isEmpty()) {
				// 名称がブランクの時はデフォルトのものへリンク。
				// Name is a link to the default ones when the blank.
	 ...
	 * The determination has been registered in the trigger item is specified
	 */
